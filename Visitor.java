...
public class Visitor extends calcBaseVisitor<Void>{
    ...
    public ArrayList<Integer> blabels=new ArrayList<Integer>();//break标记
    public ArrayList<Integer> clabels=new ArrayList<Integer>();//continue标记
    public ArrayList<ArrayList> alllist=new ArrayList<ArrayList>();//所有局部变量表
    public ArrayList<ArrayList> allarray=new ArrayList<ArrayList>();//所有局部数组表
    public ArrayList<Var> global = new ArrayList<Var>();//所有全局变量表
    public ArrayList<Narray> gloarray = new ArrayList<Narray>();//所有全局数组表
    public Funclist funclist = Funclist.getInstance();//所有函数表
    public ArrayList<param> plist = new ArrayList<param>();//对每个函数的参数表
    ...
    public Register newReg(String type){
        Register reg = new Register();
        reg.setName("%"+Num);
        reg.setNum(Num);
        reg.setType(type);
    ...
    @Override public Void visitFuncDef(calcParser.FuncDefContext ctx) {
        ...
        if(ctx.FuncType()!=null){
            results+="define dso_local i32 @";
            func.setType("int");
        }
        else {
            results+="define dso_local void @";
            func.setType("void");
        }
        results+=ctx.Idigit().getText();
        func.setName(ctx.Idigit().getText());
        if(ctx.Idigit().getText().equals("main")){
            if(!func.getType().equals("int")||ctx.funcfParams()!=null){
                System.exit(-99);
            }
            if(funclist.getFunc("main")!=null){
                System.exit(-100);
            }
        }
        funclist.add(func);
        Var var = new Var();
        var.setName(func.getName());
        var.setType("f");
        var.setNum("@" + func.getName());
    ...
    @Override
    public Void visitFuncfParams(calcParser.FuncfParamsContext ctx) {
    ...
    @Override
    public Void visitFuncfParam(calcParser.FuncfParamContext ctx) {
        param np = new param();
        np.setName(ctx.Idigit().getText());
        if(!ctx.getText().contains("[]")){
            np.setType("int");
            np.setLength("0");
            np.setRegister(newReg("i32"));
            if(!results.endsWith("(")){
                results+=",";
            }
            results+="i32 "+np.getRegister().getName();
        }
        else if(ctx.exp().size()==0){
            np.setType("array*");
            np.setLength("1");
            np.setRegister(newReg("array*"));
            if(!results.endsWith("(")){
                results+=",";
            }
            results+="i32* "+np.getRegister().getName();
    ...
    @Override public Void visitBlock(calcParser.BlockContext ctx) {
        if(infunc){
            ...
            for(int i=0;i<plist.size();i++){
                if(plist.get(i).getType().equals("int")){
                    Var var = new Var();
                    var.setName(plist.get(i).getName());
                    var.setType("i");
                    var.setNum("%" + Num);
                    var.setIsconst(false);
                    var.setInit(true);
                    ...
                    results+="store i32 "+plist.get(i).getRegister().getName()+", i32* %"+Num+"\n";
                    newReg("i32");
                }
                else if(plist.get(i).getType().equals("array*")){
                    Var var = new Var();
                    var.setName(plist.get(i).getName());
                    var.setType("a");
                    var.setNum("%" + Num);
    ...
    public String visitUnaryexp(calcParser.UnaryexpContext ctx) {
        switch (ctx.children.size()){
            ...
            default:
                ...
                if(s.equals("getint")){
                ...
                else if(s.equals("getarray")){
                    String tmp = visitFuncrParams(ctx.funcrParams());
                    results+="%"+Num+" = call i32 @getarray(i32* "+tmp+")\n";
                    Register reg = new Register();
                    reg.setName("%"+Num);
                    reg.setNum(Num);
                    reg.setType("i32");
                ...
                else if(s.equals("putarray")){
                    String tmp = visitFuncrParams(ctx.funcrParams());
                    String[] tt = tmp.split(",");
                    results+="%"+Num+" = call void @putarray(i32 "+tt[0]+",i32* "+tt[1]+")\n";
                    return null;
                }
                else if(funclist.getFunc(s)!=null){
                    Func f = funclist.getFunc(s);
                    if(f.getType().equals("int")){
                        String tmp = visitFuncrParams(ctx.funcrParams());
                        String tt[] = tmp.split(",");
                        for(int i=0;i<tt.length;i++){
                            if(Reglist.getInstance().getreg(tt[i]).getType().equals("i32")){
                                tt[i]="int "+tt[i];
                            }
                            else {
                                tt[i]="i32*"+tt[i];
                        ...
                        results+="%"+Num+" = call i32 @"+f.getName()+"(";
                        for(int i=0;i< tt.length;i++){
                            if(i!=0){
                                results+=",";
                            }
                    ...
                    else if(f.getType().equals("void")){
                        String tmp = visitFuncrParams(ctx.funcrParams());
                        results+="%"+Num+" = call void @"+f.getName()+"("+tmp+")\n";
                    }
                }
        }
        return null;
        ...
        if(ctx.exp().size()==0) {
            ...
            if (var == null) {
                ...
                if(infunc){
                    ...
                    Register npw = newReg("array*");
                    if(ctx.exp().size()==1){
                        String s=visitExp(ctx.exp(0));
                        s = regtoi32(s);
                        results +="%"+Num+" = getelementptr i32, i32* "+npw.getName()+", i32 "+s+"\n";
                        newReg("array*");
                    }
                    else {
                        String s=visitExp(ctx.exp(0));
                        s = regtoi32(s);
                        results+="%" + Num +" = mul i32 "+ s+", "+par.getLength()+"\n";
                        s=visitExp(ctx.exp(1));
                        s = regtoi32(s);
                    }
                    return "%"+(Num-1);
                }
